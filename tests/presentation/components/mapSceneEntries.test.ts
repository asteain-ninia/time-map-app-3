...
describe('collectMapSceneEntries', () => {
  ...
  it('複数レイヤー所属の地物が混在しても地図全体（layerId に依らず）で連番割当される', () => {
    // Phase 2-D-6-1+2: Layer.visible フィルタを撤去したため、layerId に関わらず
    // 全アクティブ地物が同一の連番で並ぶ。Point / LineString の自動色割当に直接影響する。
    const features = [
      new Feature('f1', 'Point', [makeAnchor('a1', 1000, 2000, pointShape('v1'), 'l1')]),
  ...
  it('Point / LineString entry の polygonRings は null', () => {
    const features = [
      new Feature('p1', 'Point', [makeAnchor('a1', 1000, 2000, pointShape('v1'), 'l1')]),
    ];
