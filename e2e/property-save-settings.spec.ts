...
test('カスタムパレットを登録すると既定パレットと地物プロパティに反映される', async ({ page }) => {
  const toolsTrigger = page.locator('.menu-trigger', { hasText: 'ツール' });
  await toolsTrigger.click();
  await page.waitForTimeout(200);
  await page.locator('.menu-action', { hasText: 'プロジェクト設定' }).click();
  ...
  await page.locator('#ps-custom-palette-name').fill('海洋');
  await page.locator('#ps-custom-palette-colors').fill('#123456, #abcdef, #7ad151');
  await page.locator('.palette-create-row button', { hasText: '追加' }).click();
  await page.locator('#ps-default-palette').selectOption('海洋');
  await page.locator('button', { hasText: '保存' }).click();
  await page.waitForTimeout(300);
  ...
  await expect(page.locator('#prop-palette')).toHaveValue('海洋');
});
