...
test('グリッドラベルはビューポート固定で横方向スクロール後も表示される', async ({ page }) => {
  const svg = page.locator('.map-svg');
  const box = await svg.boundingBox();
  if (!box) throw new Error('SVG not found');

  const longitudeLabels = page.locator('.grid-labels-overlay .longitude-label');
  const latitudeLabels = page.locator('.grid-labels-overlay .latitude-label');

  await expect(longitudeLabels.first()).toBeVisible();
  await expect(latitudeLabels.first()).toBeVisible();

  const beforeText = await longitudeLabels.allTextContents();
  ...
  const afterText = await longitudeLabels.allTextContents();
  expect(afterText.length).toBeGreaterThan(0);
  expect(afterText).not.toEqual(beforeText);
});

test('経度コンパスが上部中央に表示され、クリックで中心経度を変更できる', async ({ page }) => {
  const compass = page.locator('.longitude-compass');
  await expect(compass).toBeVisible();

  const centerValue = compass.locator('.compass-readout');
  await expect(centerValue).toContainText('0°');

  const strip = compass.locator('.compass-strip');
  const box = await strip.boundingBox();
  if (!box) throw new Error('compass strip not found');

  await page.mouse.click(box.x + box.width * 0.8, box.y + box.height / 2);
  await page.waitForTimeout(200);

  const nextText = await centerValue.textContent();
  expect(nextText).not.toBe('0°');
});
