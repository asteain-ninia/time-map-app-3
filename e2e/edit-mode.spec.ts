...
test('地物追加後に地物一覧で選択するとプロパティタブに切り替わる', async ({ page }) => {
  ...
  await expect(page.locator('#prop-id')).toBeVisible({ timeout: 3000 });
...
// §2.3.3.1 選択状態 — 地物一覧へ戻ると選択行がハイライトされる
test('地物選択後に地物一覧へ戻ると選択行がハイライトされる', async ({ page }) => {
  await addPointFeature(page);
  ...
  await expect(featureItem).toHaveClass(/selected/);
});
