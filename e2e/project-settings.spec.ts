...
test('プロジェクト設定にアプリ設定の入力欄がある', async ({ page }) => {
  const toolsTrigger = page.locator('.menu-trigger', { hasText: 'ツール' });
  await toolsTrigger.click();
  await page.waitForTimeout(200);
  const settingsAction = page.locator('.menu-action', { hasText: 'プロジェクト設定' });
  ...
  await expect(page.locator('#ps-app-snap')).toBeVisible();
  await expect(page.locator('#ps-app-fps')).toBeVisible();
  await expect(page.locator('#ps-app-vertex-limit')).toBeVisible();
});

test('アプリ設定は保存後に再読み込みしても保持される', async ({ page }) => {
  const toolsTrigger = page.locator('.menu-trigger', { hasText: 'ツール' });
  await toolsTrigger.click();
  await page.waitForTimeout(200);
  ...
  await page.locator('#ps-app-snap').fill('18');
  await page.locator('#ps-app-fps').fill('24');
  await page.locator('#ps-app-vertex-limit').fill('321');
  ...
  await expect(page.locator('#ps-app-snap')).toHaveValue('18');
  await expect(page.locator('#ps-app-fps')).toHaveValue('24');
  await expect(page.locator('#ps-app-vertex-limit')).toHaveValue('321');
});
